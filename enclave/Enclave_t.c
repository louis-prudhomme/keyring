...
static sgx_status_t SGX_CDECL sgx_sgxwasm_run_action(void* pms)
{
	...
	if (_tmp_req_bin != NULL && _len_req_bin != 0) {
		if ( _len_req_bin % sizeof(*_tmp_req_bin) != 0)
		{
		...
		if (memcpy_s(_in_req_bin, _len_req_bin, _tmp_req_bin, _len_req_bin)) {
	...
	if (_tmp_output_bin != NULL && _len_output_bin != 0) {
		if ( _len_output_bin % sizeof(*_tmp_output_bin) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
...
err:
	if (_in_req_bin) free(_in_req_bin);
	if (_in_output_bin) free(_in_output_bin);
	return status;
}
...
SGX_EXTERNC const struct {
	...
	4,
	{
		{(void*)(uintptr_t)sgx_sgxwasm_init, 0, 0},
		{(void*)(uintptr_t)sgx_sgxwasm_run_action, 0, 0},
